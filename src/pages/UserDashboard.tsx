...
export const UserDashboard: React.FC = () => {
  ...
  // Paginación para el Buscador de Productos
  ...
  const targetProduct = allProducts.find(p => p.name === selectedProduct);
  const gramsValue = Number(quantityGrams) || 0;
  const calculatedMacros = targetProduct && gramsValue > 0 ? {
    protein: Math.round(((targetProduct.proteinPer100g * gramsValue) / 100) * 10) / 10,
    carbs: Math.round(((targetProduct.carbsPer100g * gramsValue) / 100) * 10) / 10,
    fat: Math.round(((targetProduct.fatPer100g * gramsValue) / 100) * 10) / 10,
    calories: Math.round(((targetProduct.proteinPer100g * 4 + targetProduct.carbsPer100g * 4 + targetProduct.fatPer100g * 9) * gramsValue) / 100)
  } : null;

  return (
    <div className="space-y-8">
      {/* Panel de Resumen Diario con Gráfico Circular SVG */}
      <Card className="glass-panel border-none p-6 shadow-2xl flex flex-col md:flex-row items-center justify-around gap-6">
        <div className="space-y-4 text-center md:text-left flex-1 w-full">
          ...
          <p className="text-xs text-gray-400">
            *Los consumos corresponden al lote específico registrado por el QR o ingresado manualmente.
          </p>
        </div>
      ...
      {/* Modal Dialog Global de Registro de Consumo (Controlado por Estado) */}
      <Dialog open={isModalOpen} onOpenChange={(open) => {
        ...
        <DialogContent className="glass-panel border-none text-white max-w-md shadow-2xl">
          ...
          <form onSubmit={handleRegisterConsumption} className="space-y-4 pt-3">
            {targetProduct && (
              <div className="bg-white/5 border border-white/10 rounded-xl p-4 space-y-3">
                <div className="flex justify-between items-start">
                  <div>
                    <h4 className="font-bold text-white text-sm tracking-wide">{targetProduct.name}</h4>
                    <p className="text-xs text-gray-400">{targetProduct.brand} • <span className="text-primary font-medium">{targetProduct.category}</span></p>
                  </div>
                  <Badge className="bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 text-[10px] font-semibold py-0.5 px-2.5 rounded-full">
                    Lote: {activeBatches.find(b => b.id === Number(batchId))?.batchNumber || batchId}
                  </Badge>
                </div>
                ...
                {calculatedMacros && (
                  <div className="pt-2 border-t border-white/5 space-y-2">
                    ...
                    <div className="grid grid-cols-4 gap-2 text-center text-xs">
                      <div className="bg-emerald-950/40 border border-emerald-800/30 p-2 rounded-lg">
                        <span className="block text-[8px] text-emerald-400 font-bold uppercase mb-0.5">Prot</span>
                        <span className="font-bold text-white text-xs">{calculatedMacros.protein}g</span>
                      </div>
                      <div className="bg-cyan-950/40 border border-cyan-800/30 p-2 rounded-lg">
                        <span className="block text-[8px] text-cyan-400 font-bold uppercase mb-0.5">Carb</span>
                        <span className="font-bold text-white text-xs">{calculatedMacros.carbs}g</span>
                      </div>
                      <div className="bg-amber-950/40 border border-amber-800/30 p-2 rounded-lg">
                        <span className="block text-[8px] text-amber-500 font-bold uppercase mb-0.5">Grasa</span>
                        <span className="font-bold text-white text-xs">{calculatedMacros.fat}g</span>
                      </div>
                      <div className="bg-white/5 border border-white/10 p-2 rounded-lg">
                        <span className="block text-[8px] text-gray-400 font-bold uppercase mb-0.5">Kcal</span>
                        <span className="font-bold text-white text-xs">{calculatedMacros.calories}</span>
                      </div>
                    </div>
                  </div>
                )}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-gray-300">Cantidad a Consumir (en gramos)</label>
              <div className="relative flex items-center">
                <Input
                  type="number"
                  min="0.1"
                  step="0.1"
                  placeholder="ej. 30.0, 45.0"
                  value={quantityGrams}
                  onChange={(e) => setQuantityGrams(e.target.value)}
                  className="bg-[#0a0f1d] border-white/10 text-white focus:border-primary focus:ring-1 focus:ring-primary w-full pr-10 rounded-lg"
                  disabled={submitting}
                  required
                  autoFocus
                />
              ...
              <p className="text-[10px] text-gray-400">
                *Ingresa el peso exacto en gramos para calcular la ingesta nutricional y validar alérgenos en tiempo real.
              </p>
            </div>

            <Button
              type="submit"
              className="w-full bg-primary hover:bg-emerald-600 text-white font-bold py-2.5 rounded-lg mt-4 cursor-pointer transition-colors shadow-lg shadow-emerald-950/30"
              disabled={submitting}
            >
        ...
        {loadingProducts ? (
          ...
          return filteredProducts.length > 0 ? (
            <div className="space-y-4">
              ...
              {totalProductPages > 1 && (
                <div className="flex items-center justify-between px-2 pt-4 border-t border-white/5 text-xs">
                  <span className="text-gray-400">
                    Página <strong className="text-gray-200">{productPage + 1}</strong> de <strong className="text-gray-200">{totalProductPages}</strong>
                  </span>
